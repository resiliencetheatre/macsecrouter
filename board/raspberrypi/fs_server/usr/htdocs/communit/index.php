...
<body>
	...
	<h3>OTP Router</h3>
	<p>This is file transfer solution which secures tranmissions with One Time Pad (OTP) encryption.</p>
	<input id="remotefilesButton" type="button" class="w3-btn w3-orange" value="Remote files" title="remote is offline" onclick="">
	<input type="button" class="w3-btn w3-green" value="Local files" onclick="openLocalFilemanager()">
	...
	<div class="grid-item">
		...
		<p>Latency to Internet ('dns') and OTP end point:</p>
		<center>
...
function openRemoteFilemanager() {
	window.open('http://10.10.0.2/files/', '_blank');
}

function openLocalFilemanager() {
...
function $(selector) {
	...
	socket.onmessage = function(event) {
		...
		if ( otptunnel_latency == null ){
			...
			document.getElementById('remotefilesButton').setAttribute('class', 'w3-btn w3-orange');
			document.getElementById('remotefilesButton').setAttribute('title', 'Remote is offline');
			document.getElementById('remotefilesButton').setAttribute('onclick', ''); 
		} else {
			document.getElementById('remotefilesButton').setAttribute('class', 'w3-btn w3-green');
			document.getElementById('remotefilesButton').setAttribute('title', 'Open remote file manager to new window');
			document.getElementById('remotefilesButton').setAttribute('onclick', 'openRemoteFilemanager();'); 
		}
		if ( latency == null ){
			latency = 0; 
			packetLoss = 0;
	...
	wifiControlSocket.onmessage = function(event) {
		...
		if (  prefixArray[0] === "wlanstatus" ) { 
			...
			if ( wlanstatus.includes("Vault is closed") ) {
				proceedToPinEntry() ;
		}
	...
	logMessage("User interface loaded");
